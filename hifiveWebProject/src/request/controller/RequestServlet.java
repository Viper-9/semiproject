      ...
      try {
         if(profilerole.toUpperCase().equals("H")) {
            if(new SurferPartnerService().selectSurfer(loginid) != null) {
               loginrole = "S";
               if(new MatchingService().surferMatching(loginid)!= null || new MatchingService().hostMatching(profileid)!= null ){
                  job.put("result", "3");
               } else {               
            ...
            if(new HostService().selectHost(loginid) != null) {
               loginrole = "H";
               if(new MatchingService().hostMatching(loginid)!=null || new MatchingService().surferMatching(profileid)!=null){
                  job.put("result", "3");
               }
               else{
                  if(new RequestService().checkRequest(loginid, loginrole, profileid) != null) {
